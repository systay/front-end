...
class OrTypeJudgementGeneratorTest extends CypherFunSuite with AstConstructionTestSupport with BinaryOpSemanticTest {

  // shouldHandleAllSpecializations
  testValidTypes(BoolT, BoolT)(BoolT)

  // Type judgements don't need to depened on the input types
  // In this case, we know that the output type of OR cannot
